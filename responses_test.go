...
func TestUnmarshalAccountResp(t *testing.T) {
	...
	assert.Equal(t, AccountName("uosriobrazil"), resp.AccountName)
}

func TestUnmarshalBlockResp(t *testing.T) {
	RegisterAction(AccountName("uosdactokens"), ActionName("transfer"), Transfer{})
	...
	assert.Equal(t, AccountName("bitfinexuos1"), resp.Producer)
	...
	assert.Equal(t, AccountName("uosdactokens"), action.Account)
...
var accountResponseJSONData = `{
	...
	"permissions": [
		...
		"required_auth": {
			...
			"keys": [
			{
				"key": "UOS6HSE9SVvNmGF4Dv8cHLUjF8BigorYykUG2z8UbHZd1BQ9qF88r",
				"weight": 1
			}
			],
			"accounts": [],
		...
		"required_auth": {
			...
			"keys": [
			{
				"key": "UOS7FJJ7igorHoTq6y6yd7GmRei9cc6CRhR7L2TXP6H9UFEP49jNc",
				"weight": 1
			}
			],
			"accounts": [],
		...
		"required_auth": {
			...
			"keys": [
			{
				"key": "UOS5UhWBMYKPPzb4tigorbnrH9Ft7mogW1MmvViaHJkBif2kSa1f4",
				"weight": 1
			}
			],
			"accounts": [],
	...
	"total_resources": {
		"owner": "uosriobrazil",
		"net_weight": "32.4628 UOS",
		"cpu_weight": "32.9628 UOS",
		"ram_bytes": 145360
	},
	...
	"refund_request": {
		...
		"net_amount": "2.9284 UOS",
		"cpu_amount": "2.9284 UOS"
	},
	"voter_info": {
		"owner": "uosriobrazil",
		"proxy": "",
