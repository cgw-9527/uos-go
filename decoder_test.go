...
func TestDecoder_PublicKey(t *testing.T) {
	...
	pk := ecc.MustNewPublicKey("UOS1111111111111111111111111111111114T1Anm")
...
func TestDecoder_Encode(t *testing.T) {
	...
	s := &EncodeTestStruct{
		...
		F9:  ecc.MustNewPublicKey("UOS1111111111111111111111111111111114T1Anm"),
	...
	assert.Equal(t, "UOS", s.F18.Symbol.Symbol)
...
func TestDecoder_Decode_struct_tag_BinaryExtension(t *testing.T) {
	type BinaryExtensionTestStruct struct {
		S2 string
		S1 string `uos:"binary_extension"`
	}
...
func TestDecoder_Decode_struct_tag_BinaryExtension_NotGrouped(t *testing.T) {
	type BinaryExtensionTestStruct struct {
		S1 string
		S2 string `uos:"binary_extension"`
		S3 string
	}

	require.PanicsWithValue(t, "the `uos: \"binary_extension\"` tags must be packed together at the end of struct fields, problematic field S3", func() {
		var s BinaryExtensionTestStruct
...
func TestDecoder_SignedBlock_Full(t *testing.T) {
	...
	//       noduos binary data, which is usually all big endian, it is written as
	...
	//       need to craft a block with some data in it or search `noduos` to validate how a `vector<char>`
	...
	assert.Equal(t, AccountName("uosio"), signedBlock.Producer)
