...
func TestSignatureSerialization(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, `UOS5jSQLpKBHLaMtuzkftnYE6bCMA5Jxso8f22uZyKj6cDEp32eSj`, pubKey.String()) // not checked after..
...
func TestSignaturePublicKeyExtraction(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			expectedPubKey: "UOS7KtnQUSGVf4vbFE2eQsWmDp4iV93jVcSmdQXtRdRRnWj2ubbFW",
		},
		{
			name:                "R1",
			signature:           "SIG_R1_KE33Ucjr5N3GR4ZosFh8KtGMytHHNtnmdUaSoMLJVXpVXoC8B9zfoXYrLiQJZqroe3LKciaP2uJT7Myqqoo4PZH7iSnso8",
...
func TestUOSIOCSigningComparison(t *testing.T) {
	...
	assert.Equal(t, "UOS6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", pubKey.String())
}

func TestNoduosSignatureComparison(t *testing.T) {
	wif := "5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3" // corresponds to: UOS6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV
	privKey, err := NewPrivateKey(wif)
	require.NoError(t, err)
...
//to do this here because of a import cycle when use uos.SigDigest
