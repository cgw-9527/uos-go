...
$ cluos wallet private_keys -n r1test
password: [[
    "UOS67SCWnz6trqFPCtmxfjYEPSsT9JKRn4zhow8X3VTtgaEzNMULF",
...
func TestK1PrivateToPublic(t *testing.T) {
	...
	assert.Equal(t, "UOS859gxfnXyUriMgUeThh1fWv3oqcpLFyHa3TfFYC4PK2HqhToVM", pubKeyString)
}
...
func TestNewPublicKeyAndSerializeCompress(t *testing.T) {
	// Copied test from uosjs(-.*)?
	key, err := NewPublicKey("UOS6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV")
	require.NoError(t, err)
...
func TestPublicKeyValidity(t *testing.T) {
	tests := []struct {
		...
		{"UOS859gxfnXyUriMgUeThh1fWv3oqcpLFyHa3TfFYC4PK2HqhToVM", nil},
		{"MMM859gxfnXyUriMgUeThh1fWv3oqcpLFyHa3TfFYC4PK2HqhToVM", fmt.Errorf("public key should start with [\"PUB_K1_\" | \"PUB_R1_\"] (or the old \"UOS\")")},
		{"UOS859gxfnXyUriMgUeThh1fWv3oqcpLFyHa3TfFYC4PK2HqhTo", fmt.Errorf("checkDecode: invalid checksum")},
	}
