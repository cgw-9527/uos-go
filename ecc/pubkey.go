...
import (
	...
	"github.com/uoscanada/uos-go/btcsuite/btcd/btcec"
	"github.com/uoscanada/uos-go/btcsuite/btcutil/base58"
...
func NewPublicKey(pubKey string) (out PublicKey, err error) {
	...
	} else if strings.HasPrefix(pubKey, PublicKeyPrefixCompat) { // "UOS"
		pubKeyMaterial := pubKey[len(PublicKeyPrefixCompat):] // strip "UOS"
		curveID = CurveK1
		decodedPubKey, err = checkDecode(pubKeyMaterial, curveID)
		if err != nil {
			return out, fmt.Errorf("checkDecode: %s", err)
...
func checkDecode(input string, curve CurveID) (result []byte, err error) {
	...
	// perhaps bitcoin has a leading net ID / version, but UOS doesn't
...
func Ripemd160checksumHashCurve(in []byte, curve CurveID) []byte {
	...
	// proper support for importing `UOS` keys isn't rolled out into `dawn4`.
