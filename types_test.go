...
func TestInt128JSONUnmarshal(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "value from noduos serialization",
...
func TestAssetMarshalUnmarshal(t *testing.T) {
	tests := []struct {
		...
		{"808d5b000000000004454f5300000000",
			Asset{6000000, Symbol{Precision: 4, Symbol: "UOS"}}},
	}
...
func FixmeTestPackTransaction(t *testing.T) {
	...
	tx := &Transaction{
		...
		Actions: []*Action{
			{
				Account: AccountName("wxbio"),
				Name:    ActionName("transfer"),
				Authorization: []PermissionLevel{
					{AccountName("wxbio"), PermissionName("active")},
				},
			},
		},
	}
	...
	// actions: 01
	// - account: 0000000000ea3055 (wxbio)
	//   action: 000000572d3ccdcd (transfer)
	//   []auths: 01
	//     - acct: 0000000000ea3055 (wxbio)
...
func TestPackAccountName(t *testing.T) {
	// SHOULD IMPLEMENT: string_to_name from contracts/wxbiolib/types.hpp
	tests := []struct {
		in  string
		out string
	}{
...
func TestAuthorityBinaryMarshal(t *testing.T) {
	key, err := ecc.NewPublicKey("UOS6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV")
	require.NoError(t, err)
	a := Authority{
		Threshold: 2,
		Keys: []KeyWeight{
...
func TestNameToSymbol(t *testing.T) {
	tests := []struct {
		...
		{"......2ndx2k4", Symbol{Precision: 4, Symbol: "UOS"}, nil},
	}
...
func TestStringToSymbol(t *testing.T) {
	tests := []struct {
		...
		{"4,UOS", Symbol{Precision: 4, Symbol: "UOS"}, "......2ndx2k4", nil},
		{"9,UOSUOSA", Symbol{Precision: 9, Symbol: "UOSUOSA"}, "c5doylendx2kd", nil},

		{"UOS", Symbol{}, "", errors.New("UOS is not a valid symbol")},
		{",UOS", Symbol{}, "", errors.New(",UOS is not a valid symbol")},
		{"10,UOS", Symbol{}, "", errors.New("10,UOS is not a valid symbol")},
		{"10,UOS", Symbol{}, "", errors.New("10,UOS is not a valid symbol")},
		{"1,UOSUOSEO", Symbol{}, "", errors.New("1,UOSUOSEO is not a valid symbol")},
	}
...
func TestNameToSymbolCode(t *testing.T) {
	tests := []struct {
		...
		{"........ehbo5", SymbolCode(5459781), "UOS", nil},
	}
...
func TestBlob(t *testing.T) {
	...
	t.Run("Data", func(tt *testing.T) {
		data, err := b.Data()
		require.Nil(tt, err)
		assert.Equal(tt, []byte("UOS Go"), data)
	})
