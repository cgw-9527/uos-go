...
func TestActionNewAccount(t *testing.T) {
	...
	tx := &uos.Transaction{
		Actions: []*uos.Action{a},
	}

	buf, err := uos.MarshalBinary(tx)
	// println(string(buf))
...
func TestMarshalTransactionAndSigned(t *testing.T) {
	...
	tx := &uos.SignedTransaction{Transaction: &uos.Transaction{
		Actions: []*uos.Action{a},
	}}
