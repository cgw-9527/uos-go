...
type HandshakeInfo struct {
	ChainID                  uos.Checksum256
	HeadBlockNum             uint32
	HeadBlockID              uos.Checksum256
	HeadBlockTime            time.Time
	LastIrreversibleBlockNum uint32
	LastIrreversibleBlockID  uos.Checksum256
}
...
func (p *Peer) Read() (*uos.Packet, error) {
	packet, err := uos.ReadPacket(p.reader)
...
func (p *Peer) WriteP2PMessage(message uos.P2PMessage) (err error) {
...
func (p *Peer) SendSyncRequest(startBlockNum uint32, endBlockNumber uint32) (err error) {
	...
	syncRequest := &uos.SyncRequestMessage{
...
func (p *Peer) SendRequest(startBlockNum uint32, endBlockNumber uint32) (err error) {
	...
	request := &uos.RequestMessage{
		ReqTrx: uos.OrderedBlockIDs{
			Mode:    [4]byte{0, 0, 0, 0},
			Pending: startBlockNum,
		},
		ReqBlocks: uos.OrderedBlockIDs{
			Mode:    [4]byte{0, 0, 0, 0},
...
func (p *Peer) SendNotice(headBlockNum uint32, libNum uint32, mode byte) error {
	...
	notice := &uos.NoticeMessage{
		KnownTrx: uos.OrderedBlockIDs{
			Mode:    [4]byte{mode, 0, 0, 0},
			Pending: headBlockNum,
		},
		KnownBlocks: uos.OrderedBlockIDs{
			Mode:    [4]byte{mode, 0, 0, 0},
			Pending: libNum,
		},
	}
...
func (p *Peer) SendTime() error {
	...
	notice := &uos.TimeMessage{}
	return errors.WithStack(p.WriteP2PMessage(notice))
}

func (p *Peer) SendHandshake(info *HandshakeInfo) error {

	publicKey, err := ecc.NewPublicKey("UOS1111111111111111111111111111111114T1Anm")
	if err != nil {
	...
	tstamp := uos.Tstamp{Time: info.HeadBlockTime}
	...
	handshake := &uos.HandshakeMessage{
		NetworkVersion:           1206,
