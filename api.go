	...
	api := &API{
		HttpClient: &http.Client{
			Transport: &http.Transport{
				...
				DisableKeepAlives:     true, // default behavior, because of `noduos`'s lack of support for Keep alives.
			},
		},
		BaseURL:  baseURL,
...
// main `noduos` software doesn't in the version from March 22nd
...
// ProducerPause will pause block production on a noduos with
// `producer_api` plugin loaded.
func (api *API) ProducerPause() error {
	return api.call("producer", "pause", nil, nil)
}

// CreateSnapshot will write a snapshot file on a noduos with
// `producer_api` plugin loaded.
...
// ProducerResume will resume block production on a noduos with
