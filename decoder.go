...
// Decoder implements the UOS unpacking, similar to FC_BUFFER
type Decoder struct {
	data               []byte
	pos                int
...
func (d *Decoder) decodeStruct(v interface{}, t reflect.Type, rv reflect.Value) (err error) {
	...
	for i := 0; i < l; i++ {
		...
		if tag != "binary_extension" && seenBinaryExtensionField {
			panic(fmt.Sprintf("the `uos: \"binary_extension\"` tags must be packed together at the end of struct fields, problematic field %s", typeField.Name))
		}
